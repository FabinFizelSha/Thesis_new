...
std::string get_dtype_str(const std::string& encoding) {
  ...
  std::smatch m;
  // Humble does not expose the cv_type_regex helper added in newer sensor_msgs.
  static const std::regex cv_type_regex("^([0-9]+)([USF])C([0-9]+)$");
  if (std::regex_match(encoding, m, cv_type_regex)) {
    if (m.str(2) == "S") {
      prefix = "i";
    } else if (m.str(2) == "F") {
      prefix = "f";

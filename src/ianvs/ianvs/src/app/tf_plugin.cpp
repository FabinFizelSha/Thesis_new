...
void TFPlugin::on_start(rosbag2_cpp::Reader& reader,
  ...
  // ROS 2 Iron PlayOptions on this system does not expose exclude_topics_to_filter.
  // We therefore do not modify the rosbag2 exclude list here. Static TFs are still
  // read explicitly from the bag below and republished by the plugin.
  if (!checker.passes("/tf_static")) {
    return;
  }

  broadcaster_ = std::make_unique<tf2_ros::StaticTransformBroadcaster>(node_);
  // ROS 2 Iron compatibility: exclude_topics_to_filter is not available.
  // Leaving /tf_static in the original play options may duplicate static TF output,
  // but keeps ianvs/hydra_visualizer buildable and usable.

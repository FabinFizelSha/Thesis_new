...
class NodeHandle {
 public:
  ...
  rclcpp::Time now() const {
    return const_cast<NodeHandle*>(this)->clock()->now();
  }
...
typename NodeHandle::Service<T> NodeHandle::create_service(const std::string& name,
  ...
  return rclcpp::create_service<T>(
      base, services, new_name, callback, qos.get_rmw_qos_profile(), group);
}
...
typename NodeHandle::Client<T> NodeHandle::create_client(const std::string& name,
  ...
  auto services = node_.get<rclcpp::node_interfaces::NodeServicesInterface>();
  const auto new_name = join_namespace(ns_, name);
  return rclcpp::create_client<T>(
      base, graph, services, new_name, qos.get_rmw_qos_profile(), group);
}
...
NodeHandle::Timer NodeHandle::create_timer(std::chrono::milliseconds period_ms,
  ...
  auto to_return = rclcpp::create_timer(
      base, timer, clock->get_clock(), rclcpp::Duration(period_ms), callback, group);
  to_return->reset();
  return to_return;
}

#!/usr/bin/env python3
"""Summarise one Phase 1 Part 4 (SAM/tracking pipeline split) CSV.

Forked from optimisation_part3/analyse_timing.py at the start of Part 4.
Adds one new leaf stage, sam_output_queue_wait_ms -- the time a completed
SAM stage now waits in the new inter-stage FIFO before the tracking/publish
thread dequeues it. Unlike Part 1-3, which each targeted the mean cost of one
synchronous stage, Part 4 changes the *concurrency model* (SAM for frame N+1
overlaps tracking/publish for frame N) rather than any stage's own compute
cost -- so the primary question here is throughput (frames processed / drop
ratio), not which leaf stage is largest. That framing is kept below (for
continuity with Part 1-3's reports) but demoted under a new, part-specific
throughput section that is the real answer this part needs.
"""
...
def main() -> None:
    ...
    with (output_dir / "stage_summary.md").open("w", encoding="utf-8") as stream:
        stream.write("# Phase 1 optimisation Part 4 timing summary\n\n")
        stream.write(
        ...
        stream.write(
            "Part 4 changes concurrency (SAM for frame N+1 overlaps tracking/publish "
            "for frame N), not any stage's own compute cost, so the number that "
            "matters here is how many frames got through the 180 s window, not "
            "which single leaf stage is largest.\n\n"
        )
        if counts:
        ...
        stream.write(
            ...
            "question Part 4 is testing -- see the throughput section above.\n\n"
        )

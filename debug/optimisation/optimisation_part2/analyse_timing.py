#!/usr/bin/env python3
"""Summarise one Phase 1 Part 2 (geometry) CSV without ranking async RAP/VLM work.

Forked from optimisation_part1/analyse_timing.py (that part's crop-maintenance section) at the start of Part 2. The
only functional difference is the four geometry_* sub-step columns added for
Part 2 profiling: geometry_mask_extract_ms, geometry_depth_gather_ms,
geometry_projection_ms, geometry_stats_ms. They summarise like any other leaf
stage but are excluded from "largest bottleneck" candidate selection because
they are sub-components of geometry_metadata_ms, not independent stages.
"""
...
LEAF_STAGES = (
    ...
    "frame_assignment_ms", "track_association_ms", "crop_update_ms",
...
def main() -> None:
    ...
    with (output_dir / "stage_summary.md").open("w", encoding="utf-8") as stream:
        stream.write("# Phase 1 optimisation Part 2 timing summary\n\n")
        stream.write(
